...
export function parseMarketRegion(value: unknown): MarketRegion {
  return value === 'us' ? 'us' : 'cn';
}

export function normalizeStockCode(code: string, market: MarketRegion = 'cn'): string {
  if (market === 'us') {
    return normalizeUsStockCode(code);
  }
...
export function isValidUsStockCode(code: string): boolean {
  const normalizedCode = normalizeUsStockCode(code);
  if (!normalizedCode) return false;
  return US_TICKER_RE.test(normalizedCode);
}

export function isValidStockCode(code: string, market: MarketRegion): boolean {
